...
public class NodeDataGenerator {
    ...
    public static List<Node> generateFixedReal() {
        ...
        Pod pod1 = new Pod("0_0", "first_first", 600, 300,false);
        Pod pod2 = new Pod("0_1", "first_second", 800, 400,false);
        Pod pod3 = new Pod("0_2", "first_third", 400, 200,false);
        Pod pod4 = new Pod("0_3", "first_fourth", 300, 300,false);
        Pod pod5 = new Pod("0_4", "first_fifth", 700, 300,false);
        node.addPod(pod1); node.addPod(pod2); node.addPod(pod3); node.addPod(pod4); node.addPod(pod5);
        kubeEnv.add(node);
        node = new Node("1", "second", 4000, 2000);
        pod1 = new Pod("1_0", "second_first", 500, 300,false);
        pod2 = new Pod("1_1", "second_second", 400, 200,false);
        pod3 = new Pod("1_2", "second_third", 800, 400,false);
        pod4 = new Pod("1_3", "second_fourth", 1000, 300,false);
        node.addPod(pod1); node.addPod(pod2); node.addPod(pod3); node.addPod(pod4);
        kubeEnv.add(node);
        node = new Node("2", "third", 4000, 2000);
        pod1 = new Pod("2_0", "third_first", 1200, 700,false);
        pod2 = new Pod("2_1", "third_second", 500, 300,false);
        pod3 = new Pod("2_2", "third_third", 800, 400,false);
        pod4 = new Pod("1_3", "third_fourth", 300, 300,false);
        node.addPod(pod1); node.addPod(pod2); node.addPod(pod3); node.addPod(pod4);
        kubeEnv.add(node);
        return kubeEnv;
    }
