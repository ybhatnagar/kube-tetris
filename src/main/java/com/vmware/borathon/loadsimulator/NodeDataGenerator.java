...
public class NodeDataGenerator {
    ...
    public static List<Node> generateFixedReal() {
        ...
        Pod pod1 = new Pod("0_0", "first_first", 1200, 800,false);
        Pod pod2 = new Pod("0_1", "first_second", 1000, 400,false);
        Pod pod3 = new Pod("0_2", "first_third", 1000, 300,false);
        node.addPod(pod1); node.addPod(pod2); node.addPod(pod3);
        kubeEnv.add(node);
        node = new Node("1", "second", 4000, 2000);
        pod1 = new Pod("1_0", "second_first", 2000, 1000,false);
        pod2 = new Pod("1_1", "second_second", 400, 200,false);
        pod3 = new Pod("1_2", "second_third", 800, 400,false);
        node.addPod(pod1); node.addPod(pod2); node.addPod(pod3);
        kubeEnv.add(node);
        node = new Node("2", "third", 4000, 2000);
        pod1 = new Pod("2_0", "third_first", 2500, 1000,false);
